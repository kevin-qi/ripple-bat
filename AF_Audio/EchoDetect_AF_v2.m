function [click_time,click_shape,click_power,fs] = EchoDetect_AF_v2(filename)
...
threshold = 10;                                                             % Threshold (sd) for detection of events
...
if manual_cluster
    ...
    figure; set(gcf, 'units','normalized','outerposition',[0 0.2 1 0.4]);
    ...
    figure; set(gcf, 'units','normalized','outerposition',[0 0.2 0.3 0.5]);
...
for i = 1:numel(figHandles)
    saveas(figHandles(i),['Detected_Clicks', num2str(numel(figHandles)+1-i), '.png']);
    saveas(figHandles(i),['Detected_Clicks', num2str(numel(figHandles)+1-i), '.fig']);
end
close all;
...
save(['Detected_Clicks_',batdate,'.mat'],'Detected_Clicks');
