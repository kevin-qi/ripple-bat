...
%=== Tag table:
tag_SN_table = [17106917,17106926,17107055,17106969,17106904];

%=== Load data and group name
extracted_CDPfile = dir(fullfile(cd, '*extracted_*'));          load(extracted_CDPfile.name);     
if isfile('Group_name.mat'), load('Group_name.mat');    else, Group_name = 'D';end
...
elseif strcmp(extracted_CDPfile.name,'extracted_230411_cdp_fly_2.mat') || strcmp(extracted_CDPfile.name,'extracted_230412_cdp_fly_2.mat') || strcmp(extracted_CDPfile.name,'extracted_230419_cdp_fly_1.mat')
    tag_data{1,4} = tag_data{1,1};
    tag_data_filt{1,4} = tag_data_filt{1,1};
    tag_ac_data{1,4} = tag_ac_data{1,1};
end
...
if options.show_fig
    ...
    for i = 1:n_tags
        ...
        title([num2str(f_num(i)) ' flights, Tag: ',num2str(find(CDPmtdata.tag_SN(i)==tag_SN_table))]);
    end
