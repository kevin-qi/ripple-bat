function Process_Collective_Behavior_FS_AF_v0()
...
% tag_data columns::::::::::: 
% (tag_data_filt is a filtered version with similar description)
% (tag_ac_data has similar structure, with columns 3,4,5,6,7 specified below)
% COL 1:        Serial Number  
% COL 2:        Network Time (each tic is ~15.65 ps) 
% COL 3,4,5:    x(m)|y(m)|z(m) OR acceleration along the corresponding axis
% COL 6:        Signal Quality (not relevant for tag_ac_data)
% COL 7:        Number of receiving anchors (not relevant tag_ac_data)
% COL 8:        Intepolated Time (s) relative to rising edge of first TTL
...
%% Remove duplicate samples and shift Ciholas Time by 0s (t = 0 corresponds to the first '3s' Master-9 TTL)
% We used to shift by 1.05 s, in the flight room

for i = 1:n_tags
    [~,ia,~] = unique(tag_data{1,i}(:,8),'stable');         tag_data{1,i} = tag_data{1,i}(ia,:);              tag_data{1,i}(:,8) = tag_data{1,i}(:,8)+0;
    [~,ia,~] = unique(tag_data_filt{1,i}(:,8),'stable');    tag_data_filt{1,i} = tag_data_filt{1,i}(ia,:);    tag_data_filt{1,i}(:,8) = tag_data_filt{1,i}(:,8)+0;
    [~,ia,~] = unique(tag_ac_data{1,i}(:,8),'stable');      tag_ac_data{1,i} = tag_ac_data{1,i}(ia,:);        tag_ac_data{1,i}(:,8) = tag_ac_data{1,i}(:,8)+0;
end
...
if options.show_fig
    ...
    figure('units','normalized','outerposition',[0.2 0.25 0.7 0.35]);
    tiledlayout(1,n_tags,'TileSpacing','none');
    for i=1:n_tags
        nexttile;  plot3(r(:,1,i),r(:,2,i),r(:,3,i),'-','Color', bat_clr(i,:)); axis equal;  xlim(r_lim(1,:)); ylim(r_lim(2,:)); zlim(r_lim(3,:));  title(bat_nms(i,:));  view(90,90);
...
if options.savemovie
    saveCollMovie_AF_v1(r,Fs,bat_nms,r_lim,analysis_directory,[batdate,'_',Group_name]);
end
end
