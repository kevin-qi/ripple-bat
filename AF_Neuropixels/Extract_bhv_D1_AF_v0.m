...
%=== Load Data
c3d_clus_file = dir(fullfile(cd, '*10_1*.mat*'));                 c3d_data = load(c3d_clus_file.name);       % Cortex
bat_filename = strsplit(c3d_clus_file.name,'_');                  bat_filename = bat_filename{1,2};
...
options.unique_ID = {'Dataset_1','32622',batdate}; % Save the session identifier
...
if options.savedata
    analysis_directory=fullfile(pwd,['Ext_Behavior_',datestr(now, 'yymmdd_HHMMss')]);
...
[~,smp_rising] = pulsewidth(c3d_data.AnalogSignals(:,2));
...
%=== Cluster flights
alpha_clus = .7;         %Parameter for flight clustering
