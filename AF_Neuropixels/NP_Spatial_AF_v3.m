...
for hide=1
    ...
    for i=1:numel(probe_idx)
        load(['Sorted_units_AF\SU_kilosort4_outdir_probe',num2str(probe_idx(i)),'.mat']);
    ...
    load('Sorted_units_AF\Ripples_probe2.mat');                                         % This file contains Ripples from one probe
    
    %=== Load LFP from Probe1
    load('Sorted_units_AF\LFP_probe1.mat');                                             % This file contains LFP data from probe 1
    ...
    load([BHV1_file.folder,'\',BHV1_file.name],'bflying','f_num','f_smp','Fs','options','r','r_lim','t','T','v','v_abs','v_th');    % Processed Behavioral Data
        
end

%% RECLUSTER FLIGHTS
for hide=1
    alpha_clus = 0.7;   clear f_clus    % Default is 0.7
...
for hide=1
    ...
    clus_id = 2;                                            % Identity of the flight cluster for place-cell analysis
    ...
    xlabel('Time (s)'); legend('Cortex','Neuropixels');    yticks([]); title('Sampled Time');  xlim('tight');
    ...
    t_rate = [t(1):0.010:t(end)];
    all_rate = kernel_rate_AF_v1(all_s,0.05,t_rate)/n_cells;
    
end

%% ACTIVITY DURING FLIGHTS (SANITY CHECK)
for hide=1
    
    [p_rw,p_cl] = optimal_arrangement_AF_v0(min(n_cells,200));
    
    %=== Plot cells around flight time
    figure('units','normalized','outerposition',[0 0 1 1]);
    tiledlayout(p_rw,p_cl,'TileSpacing','tight');
    sgtitle('Flight Aligned Activity');
    for ncc=1:p_rw*p_cl
        nexttile;   Raster_AF_v3(s{ncc,1},t_tko ,[],[],[-1 3],[],1,1);  ylabel([]); xlabel([]);
    end
    
end

%% LOOK AT ACCELEROMETER AND LFP
for hide=1

    figure('units','normalized','outerposition',[0 .1 1 .6]);
    tiledlayout(2,1,'TileSpacing','none');
    
    ax(1) = nexttile;   plot(NP_imu.t,a_flt_NP);    ylabel('Accelerometer');    set(gca,'TickLength',[0 0]);    hold on;
    xticks([]); set(gca,'TickLength',[0 0]);
    ax(2) = nexttile;   plot(NP_imu.t,normalize(LFP_mn,'range'));   hold on;    plot(NP_imu.t,normalize(LFP_th,'range')); 
    area(t,bflying*1,0,'FaceColor',[0 0 1],'FaceAlpha',0.2,'LineStyle','none'); 
    ylabel('LFP (norm)');    set(gca,'TickLength',[0 0]);   legend('All','Theta');
    ...
    [acc_r,acc_lags] = xcorr(mean(LFP_mn,2),a_abs_NP,round(4*Fs_imu),'normalized');
...
for hide=1
    ...
    cx(3) = nexttile(6,[2 1]);   plot(NP_imu.t,LFP_mn); hold on;    plot(NP_imu.t,LFP_th);  plot(RPL.time_vector,normalize(RPL.CA1_LFP,'range')-1);      xticks([]); legend('All','Theta','SWR');  ylabel('LFP (norm)');    set(gca,'TickLength',[0 0]);
    cx(4) = nexttile(8,[2 1]);
...
for hide=1
    ...
    n2show_all = min(floor(n_place_cells/5),n2show);
        
    %=== Plot place cells around the selected cluster
    figure('units','normalized','outerposition',[0 0 1 1]);
    tiledlayout(5,4*n2show_all,'TileSpacing','tight');
    sgtitle('Example place cells');
    
    for ncc=1:(5*n2show_all)%sum(place_cond)
...
for hide=1
    ...
    for j = 2%setdiff(imp_bat_clusters,1)
        ...
        criterion = all([[NP_temp.peakHz]'>0,[NP_temp.corr_map]'>0.4],2);  % Even More Liberal
        ...
        %=== Plot place cells around the selected cluster
        n2show = min(floor(n_place_cells/5),5);                 % Make sure enough cells
        figure('units','normalized','outerposition',[0 0 1 1]);
        ...
        for ncc=1:(5*n2show)
            ...
            %=== Plot
            nexttile([1 2]);   plot3(flight_pos_rsh(1,:),flight_pos_rsh(2,:),0*flight_pos_rsh(3,:),'.','MarkerSize',1,'MarkerEdgeColor',.9*[1 1 1]);
            hold on;    textscatter(flight_pos_rsh(1,1)-0.05,flight_pos_rsh(2,1),"TKO");
            scatter3(spikes_pos(:,1),spikes_pos(:,2),spikes_pos(:,3),6,'MarkerFaceColor', 'r','MarkerEdgeColor','none','MarkerFaceAlpha',.2);
            hold off;    axis equal;
            view(0,90); xlim(r_lim(1,:)); ylim(r_lim(2,:)); zlim(r_lim(3,:));
            xticks([]); yticks([]);
        ...
        for i=1:numel(candidate_t)
            ...
            if candidate_dur(i)<0.5 && ~any(candidate_t(i)>t(f_smp(:,1)) & candidate_t(i)<t(f_smp(:,2))) && numel(vertcat(candidate_s{:}))>seq_min_spikes && candidate_perc_act(i)>seq_min_fr_active
...
for hide=1
    ...
    bx(2) = nexttile([1 1]);  plot(t_rate,all_rate,'k');  
    LFP_depth_prof = RPL.CA1_LFP_nb(:,3:end-3);
    offset_vec = [1:size(LFP_depth_prof,2)]*0.06;
    bx(3) = nexttile([1 1]);  plot(RPL.time_vector,normalize(LFP_depth_prof,1,'range')+offset_vec,'b','LineWidth',1);
    %bx(3) = nexttile([1 1]);  plot(RPL.time_vector,RPL.CA1_LFP_nb/max(RPL.CA1_LFP_nb,[],'all')+offset_vec,'b');
...
%% PLOT MANUALLY DETECTED REPLAYS
for hide=1

  load('Replay_t_manual.mat');
  [p_rw,p_cl] = optimal_arrangement_AF_v0(size(Replay_t_manual,1));
  seq_extra_t = 0;  
  
  %=== Plot replays
  figure('units','normalized','outerposition',[0 .5 .6 .5]);
  
  tiledlayout(p_rw,p_cl,'TileSpacing','compact');
  for zz=1:size(Replay_t_manual,1)
      tL = Replay_t_manual(zz,1)-seq_extra_t;
      tR = Replay_t_manual(zz,2)+seq_extra_t;
      nexttile;
      for nc= 1:n_cells_ok
...
for hide=1
    ...
    %place_cond = NP_table.pv_best<0.05 & NP_table.SI_best>1; 
    %place_cond = NP_table.pv_best<0.05 & NP_table.SI_best>.5 & NP_table.corr_m>0.4;
    place_cond = NP_table.sum_spk>-1; %& NP_table.corr_m>0.4;
    cell_place_index = find(place_cond);                       % Indexes of place cells
    n_place_cells = numel(cell_place_index);
    ...
    ax(1) = nexttile;   imagesc([t_c(1),t_c(end)],[],p_dec_global,prctile(p_dec_global,[1 99],'all')');  colormap(ax(1),'hot');  ylabel('Spatial bin');
    ax(2) = nexttile;   imagesc([t_c(1),t_c(end)],[],p_dec_filtrd,prctile(p_dec_filtrd,[1 99],'all')');  colormap(ax(2),'hot');  ylabel('Spatial bin (DS)');
    ax(3) = nexttile;   plot(t,r(:,1),'k'); hold on;
    area(t,bflying*5,0,'FaceColor',[0 0 1],'FaceAlpha',0.2,'LineStyle','none');
...
%% RUN DECODING ON CANDIDATE EVENTS (MANUALLY PICKED UP)
for hide=1
    ...
    figure('units','normalized','outerposition',[.2 .2 .6 .5]);
    for zz=1:size(Replay_t_manual,1)
        i_bin_strt = knnsearch(st_times',Replay_t_manual(zz,1));    % Find closest bin for starting
        i_bin_stop = knnsearch(st_times',Replay_t_manual(zz,2));    % Find closest bin for stopping
    ...
    %=== Run decoding on flights from that cluster
    figure('units','normalized','outerposition',[.2 .2 .6 .5]);
    t1_clus = t(f_clus.strt_frame(f_clus.id == clus_id)');
    t2_clus = t(f_clus.stop_frame(f_clus.id == clus_id)');
    for zz=1:size(t1_clus,1)
        i_bin_strt = knnsearch(st_times',t1_clus(zz));      % Find closest bin for starting
        i_bin_stop = knnsearch(st_times',t2_clus(zz));      % Find closest bin for stopping  
        p_dec_global = [];                                  % Initialize
...
for hide=1
    ...
    ext_flight = movmax(bflying,Fs*0.2);                                         % Extend flight around flight tails
    ...
    bx(2) = nexttile;   plot(t,r(:,1),'k'); hold on;
    area(t,bflying*5,0,'FaceColor',[0 0 1],'FaceAlpha',0.2,'LineStyle','none');
    plot((t(f_clus.strt_frame(f_clus.id == clus_id)')*[1 1])',(ones(size(f_clus.strt_frame(f_clus.id == clus_id)'))*[0 5])','r');
    ylabel('x (m)');
