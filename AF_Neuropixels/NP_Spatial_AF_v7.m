function NP_Spatial_AF_v7(folder_name)
...
for hide=1
    ...
    load('merges.mat');                                                                 % This contains the ids of potential merges
...
for hide=1
    ...
    for j = setdiff(imp_bat_clusters,1)
        ...
        for ncc=1:n2show
            ...
            [~,s_flight] = count_spikes_AF_v1(s{nc,1},t,[smp1_clus-round(0.5*Fs) smp2_clus+1+round(0.5*Fs)]);
            [ac_TMI,ac_TMI_bins] = cross_correlogram_AF_v0(s_flight,s_flight,max_lag_TMI,bin_TMI);
            ...
            yticks([]); xlabel('Time lag (s)'); title(['TMI: ',num2str(NP_unit(nc).TMI(1),2), ', p = ',num2str(NP_unit(nc).p_val_TMI,1)]);
        ...
        for zz=cRT_subset'
            ...
            xticks([tL-0.05, tR+0.05]);   xticklabels({'',[num2str((tR-tL)*1000,3), ' ms']});
            i=i+1;
        end
        sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
        fig_count = saveFig(analysis_directory,[cell2mat(unique_ID)],fig_count,options.savefigures);
...
endfunction NP_Spatial_AF_v8(folder_name)
%% SCRIPT FOR LOOKING AT NP RECORDINGS WITH MULTIPLE PROBES IN THE HIPPOCAMPUS DURING NAVIGATION
% Updated by A.F. on May 2024, based on previous versions
% BRIEF DESCRIPTION (TBD)
...
for hide=1
    ...
    a_mdg = interp1(t_Hi,movmean(abs(a_abs_NP-1),Fs_Hi*0.5),t,'linear',0);  % mean deviation from g in a 0.5s integration window
    immobile = a_mdg<0.04;                                  % Immobility vector
    wb_phase = angle(hilbert(a_flt_NP));                    % wingbeat phase
...
for hide=1
    ...
    for j = setdiff(imp_bat_clusters,1)
        ...
        for nc = 1:n_place_cells,[~,s_flight{nc,1}] = count_spikes_AF_v0(s{sorted_tko_plcCells(nc),1},t,[smp1_clus smp2_clus]);end
        s_flight1 = cell(n_cells,1);            % Spikes during the flight cluster (unsorted, extended)
        for nc = 1:n_cells,[~,s_flight1{nc,1}] = count_spikes_AF_v0(s{nc,1},t,[smp1_clus+t_ic(1)*Fs smp2_clus+t_ic(2)*Fs]);end
      
        %=== Extract a few variables for phase precession
        %wb_phase_flip = flip(wb_phase);
        ...
        for nc = 1:n_place_cells
            ...
            for nspk = 1:numel(s_flight{nc,1})
                tmp_idx = knnsearch_fast_AF_v0(t,s_flight{nc,1}(nspk,1),0.05);
                phpl_flight{nc,1}(nspk) = d2place(tmp_idx);
                tmp_idx = knnsearch_fast_AF_v0(t_Hi,s_flight{nc,1}(nspk,1),0.05);
                phpl_flight{nc,2}(nspk) = wb_phase(tmp_idx);
                %phpl_flight{nc,2}(nspk) = wb_phase_flip(tmp_idx);
            end
        end 
        ...
        for ncc=1:n2show
            ...
            view(0,90); xlim(r_lim(1,:)); ylim(r_lim(2,:)); zlim(r_lim(3,:));
            xticks([]); yticks([]); title(['Unit ' num2str(nc)]);
            nexttile([1 2]); scatter(phpl_flight{ncc,2},phpl_flight{ncc,1},6,'filled','MarkerFaceColor','k');
            xlim(prctile(phpl_flight{ncc,2},[1 99])*1.3);   ylim(prctile(phpl_flight{ncc,1},[1 99])*1.3);
...
for hide=1
    for j = setdiff(imp_bat_clusters,1)
        ...
        for nc = 1:n_dec_cells,[~,s_flight2{nc,1}] = count_spikes_AF_v0(s{sorted_tko_plcCells(nc),1},t,[smp1_clus smp2_clus]);end
