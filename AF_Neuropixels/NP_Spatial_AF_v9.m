function NP_Spatial_AF_v9(folder_name)
...
for hide=1
    ...
    %scatter3(r(logical(bflying),1),r(logical(bflying),2),r(logical(bflying),3),'filled','MarkerFaceAlpha',0.1,'MarkerEdgeColor','none');   view([0,90]);
end
...
for hide=1
    ...
    d_thr_F = 0.75;                                         % Distance from feeder for flight classification
    ...
    if strcmp(unique_ID{1,1},'Dataset_2')
        r_fd = [-2.93,0.69,1.85; 100,100,100];                                      % Correct coordinates for Field Station
    end
    ...
    %=== Classify type of flight (Perch-Feeder,Feeder-Perch,Feeder-Feeder,Perch-Perch)
    figure('units','normalized','outerposition',[.35 .3 .4 .5],'Visible',fig_visibility);
    tiledlayout(1,4,'TileSpacing','tight');
    zx1 = nexttile; title(zx1, 'Feeder-Feeder'); hold on;
    zx2 = nexttile; title(zx2, 'Feeder-Perch'); hold on;
    zx3 = nexttile; title(zx3, 'Perch-Feeder'); hold on;
    zx4 = nexttile; title(zx4, 'Perch-Perch'); hold on;
    for i=1:f_clus.N
        d_start1 = vecnorm(r_fd(1,:) - r(f_clus.strt_frame(i),:));
        d_start2 = vecnorm(r_fd(2,:) - r(f_clus.strt_frame(i),:));
        d_stop1  = vecnorm(r_fd(1,:) - r(f_clus.stop_frame(i),:));
        d_stop2  = vecnorm(r_fd(2,:) - r(f_clus.stop_frame(i),:));
        if (d_start1 < d_thr_F || d_start2 < d_thr_F) && (d_stop1 < d_thr_F || d_stop2 < d_thr_F)
            f_clus.type(i) = {'FF'};
            axes(zx1); plot(f_clus.pos(1,:,i),f_clus.pos(2,:,i),'g');   axis equal; xlim(r_lim(1,:)); ylim(r_lim(2,:)); 
            textscatter(r(f_clus.strt_frame(i),1),r(f_clus.strt_frame(i),2),"x"); 
        elseif (d_start1 < d_thr_F || d_start2 < d_thr_F) && (d_stop1 > d_thr_F && d_stop2 > d_thr_F)
            f_clus.type(i) = {'FP'};
            axes(zx2);  plot(f_clus.pos(1,:,i),f_clus.pos(2,:,i),'b');  axis equal; xlim(r_lim(1,:)); ylim(r_lim(2,:));
            textscatter(r(f_clus.strt_frame(i),1),r(f_clus.strt_frame(i),2),"x");
        elseif (d_start1 > d_thr_F && d_start2 > d_thr_F) && (d_stop1 < d_thr_F || d_stop2 < d_thr_F)
            f_clus.type(i) = {'PF'};
            axes(zx3);  plot(f_clus.pos(1,:,i),f_clus.pos(2,:,i),'r');  axis equal; xlim(r_lim(1,:)); ylim(r_lim(2,:));
            textscatter(r(f_clus.strt_frame(i),1),r(f_clus.strt_frame(i),2),"x");
        else
            f_clus.type(i) = {'PP'};
            axes(zx4);  plot(f_clus.pos(1,:,i),f_clus.pos(2,:,i),'k');  axis equal; xlim(r_lim(1,:)); ylim(r_lim(2,:));
            textscatter(r(f_clus.strt_frame(i),1),r(f_clus.strt_frame(i),2),"x");
        end
    end
    sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
...
for hide=1
    ...
    for j = setdiff(imp_bat_clusters,1)
        ...
        % Find cluster type
        [uniqueStrings, ~, idx] = unique(f_clus.type(f_clus.id==j));
        counts_tmp = accumarray(idx, 1);[~, maxIdx] = max(counts_tmp); clus_type_String = uniqueStrings{maxIdx};
        ...
        Rpl_table_tmp.clus_type = repmat({clus_type_String},n_candidate_events,1);                                    % Cluster type (Feeder-Feeder, etc...)
        ...
        [~,Rpl_table_tmp.t2pvs] = knnsearch(t(f_smp(:,2)),Rpl_table_tmp.tL);                                        % Time from nearest takeoff from any cluster
        [~,Rpl_table_tmp.t2nxt] = knnsearch(t(f_smp(:,1)),Rpl_table_tmp.tL);                                        % Time from nearest landing from any cluster
        ...
        Rpl_table_tmp.clus_seq = repmat([1 1 1],n_candidate_events,1);                                              % Sequence of previous-replayed-next cluster
        for rr=1:size(Rpl_table_tmp,1)
            ...
            prev_tmp = find(f_clus.stop_frame<Rpl_table_tmp.smpL(rr));
            if ~isempty(prev_tmp),prev_clus = f_clus.id(prev_tmp(end));else,prev_clus = NaN;end
            next_tmp = find(f_clus.stop_frame>Rpl_table_tmp.smpL(rr));
            if ~isempty(next_tmp),next_clus = f_clus.id(next_tmp(1  ));else,next_clus = NaN;end
            Rpl_table_tmp.clus_seq(rr,:) = [prev_clus j next_clus];
        end
        ...
        temp_seq = diff(Rpl_table_tmp.clus_seq,1,2);    lookup = {'BT', 'IN', 'IP', 'OT'};                          % Replay type: Both, Immediate Next, Immediate Previous, Other
        repl_type = @(x, y) cellfun(@(c) c, lookup((x~=0) + 2*(y~=0) + 1), 'UniformOutput', false);                 
        Rpl_table_tmp.rpl_type = arrayfun(@(i) repl_type(temp_seq(i,1), temp_seq(i,2)), (1:size(temp_seq,1))', 'UniformOutput', false);
        ...
        %=== Plot replay tipe: of immediate previous, immediate next or other
        figure('units','normalized','outerposition',[.3 .2 .15 .4],'Visible',fig_visibility);
        bar(sum([strcmp([Rpl_subtable.rpl_type{:}],'IP')',strcmp([Rpl_subtable.rpl_type{:}],'IN')',strcmp([Rpl_subtable.rpl_type{:}],'BT')',strcmp([Rpl_subtable.rpl_type{:}],'OT')']),'FaceColor',col_clus(j,:));
        xticks([1:4]);        xticklabels({'Immediate Previous','Immediate Next','Both','Other'});  ylabel('Count of Replay Type');
        sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
        fig_count = saveFig(analysis_directory,[cell2mat(unique_ID)],fig_count,options.savefigures);
...
for hide=1
    ...
    %=== Plot replay tipe: of immediate previous, immediate next or other and feeder type
    Rpl_subtable = Rpl_table(Rpl_table.n>4 & Rpl_table.p<0.05,:);
    figure('units','normalized','outerposition',[.3 .2 .3 .4],'Visible',fig_visibility);
    tiledlayout(1,2,'TileSpacing','tight');
    nexttile;   bar(sum([strcmp([Rpl_subtable.rpl_type{:}],'IP')',strcmp([Rpl_subtable.rpl_type{:}],'IN')',strcmp([Rpl_subtable.rpl_type{:}],'BT')',strcmp([Rpl_subtable.rpl_type{:}],'OT')']),'FaceColor','k');
    xticks([1:4]);        xticklabels({'Immediate Previous','Immediate Next','Both','Other'});  ylabel('Count of Replay Type');
    nexttile;   bar(sum([strcmp(vertcat(Rpl_subtable.clus_type),'FF'),strcmp(vertcat(Rpl_subtable.clus_type),'PF'),strcmp(vertcat(Rpl_subtable.clus_type),'FP'),strcmp(vertcat(Rpl_subtable.clus_type),'PP')]),'FaceColor','r');
    xticks([1:4]);        xticklabels({'Feeder-Feeder','Perch-Feeder','Feeder-Perch','Perch-Perch'});  ylabel('Count of Replay Cluster Type');
    sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
    fig_count = saveFig(analysis_directory,[cell2mat(unique_ID)],fig_count,options.savefigures);
...
for hide=1
    if options.savedata
        save([analysis_directory,'/Analyzed_NPs_', unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3},'.mat'],'Rpl_table','NP_unit');
    end
end
