function NP_Spatial_Theta_AF_v1(folder_name)
...
for hide=1
    ...
    elseif exist('folder_name') && options.savedata
        if ~contains(folder_name,'D4')
            analysis_directory = [replace(pwd,'NP_datasets',folder_name),'\NP&BHv_Analysis_',datestr(now, 'yymmdd_HHMM')];
            if ~exist(analysis_directory,'dir');mkdir(analysis_directory);end
        else
            analysis_directory = [replace(pwd,'NP_new_datasets',folder_name),'\NP&BHv_Analysis_',datestr(now, 'yymmdd_HHMM')];
            if ~exist(analysis_directory,'dir');mkdir(analysis_directory);end
        end
...
for hide=1
    ...
    %=== Store accelerometer and LFP from all flights
    int_t = [-3 5]; interval = [round(int_t(1)*Fs_Hi) : round(int_t(2)*Fs_Hi)-1];
    temp_store1 = [];   temp_store2 = [];
    for i=1:f_num_Hi
        zoom_int = f_smp_Hi(i,1)+interval;
        if f_smp_Hi(i,1)+interval(1)>0 && f_smp_Hi(i,1)+interval(end)<numel(LFP)
            temp_store1 = [temp_store1; LFP(zoom_int)'];
            temp_store2 = [temp_store2; a_abs_NP(zoom_int)'];
        end
    end
    LFP_PSD.interval = {interval./Fs_Hi};
    LFP_PSD.LFPATflight = {temp_store1};
    LFP_PSD.accATflight = {temp_store2};
...
for hide=1
    ...
    [up_wbt_env,lo_wbt_env] = envelope(a_flt_NP,round(0.06*Fs_Hi),'peak');
    [up_lfp_env,lo_lfp_env] = envelope(LFP_tht,round(0.06*Fs_Hi),'peak');
    amp_wbt_env = up_wbt_env - lo_wbt_env;
    amp_lfp_env = smoothdata(up_lfp_env - lo_lfp_env,'gaussian',round(1*Fs_Hi));
    [cor_LFP2acc,lags] = xcorr(a_abs_NP(bflying_Hi),LFP_f,round(2*Fs_Hi));
    [cor_LFP2acc_env,lags] = xcorr(amp_wbt_env(bflying_Hi),amp_lfp_env(bflying_Hi),round(2*Fs_Hi));
    [coh, freq_coh] = mscohere(LFP_f,a_abs_NP(bflying_Hi), [], [], [], Fs_Hi);
    %wb_maxima = wbt_phase>0.99*pi;     % Find wingbeat maxima
    [~,wb_maxima] = findpeaks(a_flt_NP,'MinPeakHeight',0.5,'MinPeakDistance',round(0.06*Fs_Hi));
    [~,max_tmp] = max(cor_LFP2acc_env);     % Find max correlation
    
    plot(t_Hi,normalize(amp_wbt_env,'scale'),t_Hi,normalize(amp_lfp_env,'scale'));  hold on;  area(t,10*bflying,0,'FaceColor',[0 0 1],'FaceAlpha',0.5,'LineStyle','none');
    
    
    figure('units','normalized','outerposition',[0.1 0.3 0.5 0.3]);
    ...
    nexttile;   plot(lags./Fs_Hi,cor_LFP2acc_env);  hold on;    plot([0 0],ylim);   xlim('tight');  xlabel('Time lag'); ylabel('Correlation Acc-LFP (envelopes)');
    nexttile;   plot(freq_coh,smoothdata(coh,'movmedian',Fs_Hi*0.05));  xlabel('Frequency (Hz)');   ylabel('Coherence Acc-LFP');    xlim([0 20]);    hold on;    plot([f_wBeats f_wBeats],ylim,'k--');  plot(2*[f_wBeats f_wBeats],ylim,'k--');
    nexttile;   scatter(amp_wbt_env(wb_maxima),amp_lfp_env(wb_maxima+lags(max_tmp)),20,'MarkerFaceColor', 'r','MarkerEdgeColor','none','MarkerFaceAlpha',.2);    
    axis square;    lsline; hold on; plot(xlim,mean(amp_lfp_env).*[1 1]);
    [c_temp,p_temp] = corr(amp_wbt_env(wb_maxima),amp_lfp_env(wb_maxima+lags(max_tmp)));
    title([c_temp,p_temp]); xlabel('Accelerometer Envelope');   ylabel('LFP Envelope (Shifted by Optimal Lag');
    nexttile;   scatter(a_abs_NP(wb_maxima),-LFP(wb_maxima+lags(max_tmp)),6,'MarkerFaceColor', 'r','MarkerEdgeColor','none','MarkerFaceAlpha',.2);
    ylim(prctile(-LFP(wb_maxima+lags(max_tmp)),[2 98]));   xlim(prctile(a_abs_NP(wb_maxima),[2 98]));   axis square;  lsline;
    sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
    fig_count = saveFig(analysis_directory,[cell2mat(unique_ID)],fig_count,options.savefigures);
    ...
    figure('units','normalized','outerposition',[.3 0.3 .25 .35]);
    tiledlayout(1,2,'TileSpacing','tight');
    nexttile;
    ...
    figure('units','normalized','outerposition',[.3 .2 .2 .4]);
    ...
    tt2dt = LFP_PSD.theta2delta_flgt;
    dur_pre_diff = LFP_PSD.delta_tht_DurPre;
    fract_flights_wTht = sum(LFP_PSD.delta_tht_DurPre>0 & LFP_PSD.p_val_tht_DurPre<0.05 & LFP_PSD.theta2delta_flgt>2)./LFP_PSD.f_num;
    pwr_b = LFP_PSD.perc_thtpower_b;
    pwr_f = LFP_PSD.perc_thtpower_f;
    disp([num2str(fract_flights_wTht*sum(LFP_PSD.f_num),3),' out of ', num2str(sum(LFP_PSD.f_num),4), ' flights with theta (', num2str(fract_flights_wTht,3),')' ]);
    disp(['Fraction bouts during rest: ', num2str(1-mean([LFP_PSD.fract_bouts_flight]),3)]);
...
%     histogram(vertcat(LFP_PSD.tht_bouts_dur),'Normalization','probability','facealpha',.5,'edgecolor','none','FaceColor','k');
...
for hide=1
    ...
    for nc=1:n_cells
        ...
        %=== Real data
        spk_phase2wbt_F = wbt_phase(s_smp_flight_hi{nc,1});
        spk_phase2tht_F = tht_phase(s_smp_flight_hi{nc,1});   power_thtAtspike_F =  tht_power(s_smp_flight_hi{nc,1});  spk_phase2tht_F = spk_phase2tht_F(power_thtAtspike_F>prctile(power_thtAtspike_F,min_prctile_power));
        spk_phase2tmr_F = tmr_phase(s_smp_flight_hi{nc,1});   power_tmrAtspike_F =  tmr_power(s_smp_flight_hi{nc,1});  spk_phase2tmr_F = spk_phase2tmr_F(power_tmrAtspike_F>prctile(power_tmrAtspike_F,min_prctile_power));
        spk_phase2tht_A = tht_phase(s_smp_hi{nc,1});          power_thtAtspike_A =  tht_power(s_smp_hi{nc,1});         spk_phase2tht_A = spk_phase2tht_A(power_thtAtspike_A>prctile(power_thtAtspike_A,min_prctile_power));
        spk_phase2tmr_A = tmr_phase(s_smp_hi{nc,1});          power_tmrAtspike_A =  tmr_power(s_smp_hi{nc,1});         spk_phase2tmr_A = spk_phase2tmr_A(power_tmrAtspike_A>prctile(power_tmrAtspike_A,min_prctile_power));
    ...
    %=== Show population phase locking
    pop_phase2wbt_F =  mean(horzcat(NP_unit(:).spk_phase2wbt_F_counts),2,'omitnan')';
    pop_phase2tht_F =  mean(horzcat(NP_unit(:).spk_phase2tht_F_counts),2,'omitnan')';
    pop_phase2tmr_F =  mean(horzcat(NP_unit(:).spk_phase2tmr_F_counts),2,'omitnan')';
    pop_phase2tht_A =  mean(horzcat(NP_unit(:).spk_phase2tht_A_counts),2,'omitnan')';
    pop_phase2tmr_A =  mean(horzcat(NP_unit(:).spk_phase2tmr_A_counts),2,'omitnan')';
    figure('units','normalized','outerposition',[.2 .2 0.4 0.3]);
    tiledlayout(1,5,'TileSpacing','none');
    nexttile;   bar([phase_ctrs,phase_ctrs+2*pi],[pop_phase2wbt_F,pop_phase2wbt_F],1,'k');    yticks([]); xticks(-pi:pi:3*pi);    xticklabels({'-180', '0', '180', '360', '540'});   xlabel('Wingbeat');
    nexttile;   bar([phase_ctrs,phase_ctrs+2*pi],[pop_phase2tht_F,pop_phase2tht_F],1,'r');    yticks([]); xticks(-pi:pi:3*pi);    xticklabels({'-180', '0', '180', '360', '540'});   xlabel('Theta (F)');
    nexttile;   bar([phase_ctrs,phase_ctrs+2*pi],[pop_phase2tmr_F,pop_phase2tmr_F],1,'g');    yticks([]); xticks(-pi:pi:3*pi);    xticklabels({'-180', '0', '180', '360', '540'});   xlabel('Tamir (F)');
    nexttile;   bar([phase_ctrs,phase_ctrs+2*pi],[pop_phase2tht_A,pop_phase2tht_A],1,'r');    yticks([]); xticks(-pi:pi:3*pi);    xticklabels({'-180', '0', '180', '360', '540'});   xlabel('Theta (A)');
    nexttile;   bar([phase_ctrs,phase_ctrs+2*pi],[pop_phase2tmr_A,pop_phase2tmr_A],1,'g');    yticks([]); xticks(-pi:pi:3*pi);    xticklabels({'-180', '0', '180', '360', '540'});   xlabel('Tamir (A)');
    sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3}],'Interpreter','none');
    fig_count = saveFig(analysis_directory,[cell2mat(unique_ID)],fig_count,options.savefigures);
...
for hide=1
    ...
    for j = setdiff(imp_bat_clusters,1)
        ...
        figure('units','normalized','outerposition',[.2 .2 0.2 0.4]);
        ...
        figure('units','normalized','outerposition',[0.3 .3 0.17 .4]);
...
for hide=1
    for j = setdiff(imp_bat_clusters,1)
        ...
        for zz=1:n_fclus
            ...
            tht_SWPS(zz).r_real = zeros(n_bins_tmp,3);                  % Initialize real positional vector
            ...
            for i_dec = i_bin_strt:i_bin_stop
                ...
                tht_SWPS(zz).r_real(counter,:) = flight_pos(:,smpTotko,zz)';                                % Real positional vector
