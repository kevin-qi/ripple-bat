function NP_Theta_Eval_AF_v1(probe_number,folder_name)
...
if ~exist('folder_name') && options.savedata
    analysis_directory = [replace(pwd,'NP_datasets','NP_datasets_Analysis_Figures'),'\NP&BHv_Analysis',datestr(now, 'yymmdd_HHMM'),'_probe_',num2str(probe_number)];
    if ~exist(analysis_directory,'dir');mkdir(analysis_directory);end
elseif exist('folder_name') && options.savedata
    analysis_directory = [replace(pwd,'NP_datasets',folder_name),'\NP&BHv_Analysis_',datestr(now, 'yymmdd_HHMM'),'_probe_',num2str(probe_number)];
    if ~exist(analysis_directory,'dir');mkdir(analysis_directory);end
...
%=== Transform the LFP into double and interpolate at even sampling and at imu sampling
...
for i=1:f_num
    if t(f_smp(i,1))>=NP_imu.t(1) && t(f_smp(i,2))<=NP_imu.t(end)
        f_smp(i,1) = knnsearch_fast_AF_v0(NP_imu.t',t(f_smp(i,1)),0.1);
        f_smp(i,2) = knnsearch_fast_AF_v0(NP_imu.t',t(f_smp(i,2)),0.1);
    else
        f_smp(i,1) = NaN;
        f_smp(i,2) = NaN;
    end
end
f_smp = f_smp(~isnan(f_smp(:,1)),:);    f_num = size(f_smp,1);

%=== LFP and PSD 
LFP_r = LFP_interp(~wBeats,:);      [PSD_r,f_PSD_r] = pwelch(LFP_r,[],[],[],Fs_imu);  PSD_r = PSD_r./sum(PSD_r,1);  f_PSD_smpl_r = mean(diff(f_PSD_r)); % Rest
LFP_f = LFP_interp( wBeats,:);      [PSD_f,f_PSD_f] = pwelch(LFP_f,[],[],[],Fs_imu);  PSD_f = PSD_f./sum(PSD_f,1);  f_PSD_smpl_f = mean(diff(f_PSD_f)); % Flight
LFP_i = LFP_interp(a_mdg<0.04,:);   if isempty(LFP_i), LFP_i=LFP_r;end
[PSD_i,f_PSD_i] = pwelch(LFP_i,[],[],[],Fs_imu);  PSD_i = PSD_i./sum(PSD_i,1);  f_PSD_smpl_i = mean(diff(f_PSD_i)); % Immobile
...
Probe.theta = sum(PSD_r(f_PSD_r>Tht_rng(1) & f_PSD_r<Tht_rng(2),:))'; 
Probe.theta_i = sum(PSD_i(f_PSD_i>Tht_rng(1) & f_PSD_i<Tht_rng(2),:))';
Probe.theta_f = sum(PSD_f(f_PSD_f>Tht_rng(1) & f_PSD_f<Tht_rng(2),:))'; 
Probe.ripple = sum(PSD_r(f_PSD_r>Swr_rng(1) & f_PSD_r<Swr_rng(2),:))'; 
Probe.delta = sum(PSD_r(f_PSD_r>Dlt_rng(1) & f_PSD_r<Dlt_rng(2),:))'; 
...
opt_int = intersect(1:numel(Probe.tmp),tmp_idx+[-2:2]);

%=== Save locally the best probe and channel
best_prb_ch = [probe_number,tmp_idx];
save(['BPC_',unique_ID{1, 1},'_',unique_ID{1, 2},'_',unique_ID{1, 3}],'best_prb_ch');
...
NP_dlt = NaN(row_idx(end),4);   NP_dlt(sub2ind(size(NP_dlt),row_idx,col_idx)) = Probe.delta;
...
figure('units','normalized','outerposition',[.3 .1 .5 .6]);
...
nexttile;   imagesc(NP_dlt,'AlphaData',~isnan(NP_dlt));   set(gca,'YTickLabel',[],'XTickLabel',[],'YDir','normal'); axis('off');   title('Rel Delta Pwr');  
...
rectangle('Position',[t(1) (N_channels-opt_int(end)+1)*scale_f t(end)-t(1) (opt_int(end)-opt_int(1))*scale_f],'FaceColor',[1 0 0 0.2],'EdgeColor','none');
ylim('tight');  ylabel('LFP');  xticks([]);
Y_labels = num2str(flip([[1:N_channels]',row_idx,col_idx,red_out.channelMap'],1));                    
...
sgtitle([unique_ID{1},', Bat ',unique_ID{2},', ',unique_ID{3},', Probe ',num2str(probe_number),'Best Ch:', num2str(tmp_idx)],'Interpreter','none');

if options.savedata
    figHandles = findall(0,'Type','figure');
    for i = 1:numel(figHandles)
        saveas(figHandles(i),[analysis_directory, '\Figure', num2str(numel(figHandles)+1-i), '.fig']);
    end
end
fig_count = saveFig(analysis_directory,cell2mat(unique_ID),fig_count,options.savefigures);


% %=== Verify optimal channel
% tmp_idx = input('Best Channel?');
% Probe.tmp = 0*Probe.theta_f;    Probe.tmp(tmp_idx) = 1;
% opt_int = intersect(1:numel(Probe.tmp),tmp_idx+[-2:2]);
...
%=== Define LFP at best channels and filter in theta range
LFP_tht = bandpass(mean(LFP_interp(:,opt_int),2),Tht_rng,Fs_imu);
LFP_dlt = bandpass(mean(LFP_interp(:,opt_int),2),Dlt_rng,Fs_imu);
LFP_ave = mean(LFP_interp(:,opt_int),2);

%=== Calculate theta power and delta power across sliding window
tht_win_dur = 2.1;  % Theta window duration
tht_win_stp = 0.2;  % Step size for moving the window
tht_win_samples = [1:tht_win_dur*NP_imu.Fs];
n_tht_win_samples = numel(tht_win_samples);
tht_win_starts = [NP_imu.t(1):tht_win_stp:NP_imu.t(end)-tht_win_dur];
n_tht_windows = numel(tht_win_starts);
LFP_cell = cell(n_tht_windows,1);  LFP_t_cell = cell(n_tht_windows,1); 
for i=1:n_tht_windows
    idx_tmp = knnsearch_fast_AF_v0(NP_imu.t,tht_win_starts(i),0.01);
    LFP_cell{i,1} = LFP_ave(idx_tmp+tht_win_samples-1);
    LFP_t_cell{i,1} = NP_imu.t(idx_tmp+tht_win_samples-1);
end
PSD_cell = cellfun(@(x)  pwelch(x,1024,[],[],NP_imu.Fs),LFP_cell,'UniformOutput',false);    % Power Spectral density for each window
[~,PSD_freqs] = pwelch(LFP_cell{1,1},1024,[],[],NP_imu.Fs);                                 % Frequencies for the PSD
Theta_cell = cellfun(@(x) mean(x(PSD_freqs>Tht_rng(1) & PSD_freqs<Tht_rng(2))),PSD_cell);   % Theta power (4-8 Hz)
Delta_cell = cellfun(@(x) mean(x(PSD_freqs>Dlt_rng(1) & PSD_freqs<Dlt_rng(2))),PSD_cell);   % Delta power (2-4 Hz)
Theta2Delta = Theta_cell./Delta_cell;                                                       % Theta to delta ratio
LFP_theta_t = cellfun(@mean ,LFP_t_cell);                                                   % Central time for each segment
...
bx(1) = nexttile;       plot(NP_imu.t,a_flt_NP);  xticks([]); set(gca,'TickLength',[0 0]);    ylabel('Accelerometer');     
bx(2) = nexttile;       plot(NP_imu.t,LFP_ave);   xticks([]); set(gca,'TickLength',[0 0]);    ylabel('LFP All');   
bx(3) = nexttile;       plot(NP_imu.t,LFP_tht);   xticks([]); set(gca,'TickLength',[0 0]);    ylabel('LFP Theta'); 
bx(4) = nexttile;       plot(NP_imu.t,LFP_dlt);   xticks([]); set(gca,'TickLength',[0 0]);    ylabel('LFP Delta'); 
bx(5) = nexttile;       plot(LFP_theta_t,Theta2Delta);                                        ylabel('Theta/Delta ratio'); 
