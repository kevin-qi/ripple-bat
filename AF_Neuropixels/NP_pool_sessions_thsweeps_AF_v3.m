...
for hide=1
    ...
    %SWP_sst = single_SWP(single_SWP.mean_fract_pos>0.15 & single_SWP.mean_fract_pos<0.85 & single_SWP.mean_spk_dsty>prctile(single_SWP.mean_spk_dsty,30) & single_SWP.mean_jmp_distance>0.1 & single_SWP.prc_dec>0.5 & single_SWP.rms_dec<1.5 & single_SWP.med_max_post>0.1,:);
    min_pos1 = 0.15;
    min_pos2 = 0.85;
    min_rms = 1.3;
    min_prc = 0.8;
    ...
    avg_est_dist_sh = mean(est_dist_sh,2);            sem_est_dist_sh = std(est_dist_sh,[],2)/sqrt(N_sweeps_sh);
    avg_rsz_spk_dsty_sh = mean(rsz_spk_dsty_sh,2);    sem_rsz_spk_dsty_sh = std(rsz_spk_dsty_sh,[],2)/sqrt(N_sweeps_sh);
    avg_rsz_wbt_sh = mean(rsz_wbt_sh,2);              sem_rsz_wbt_sh = std(rsz_wbt_sh,[],2)/sqrt(N_sweeps_sh);
    avg_rsz_LFP_sh = mean(rsz_LFP_sh,2);              sem_rsz_LFP_sh = std(rsz_LFP_sh,[],2)/sqrt(N_sweeps_sh);
    avg_rsz_wbt_phase_sh = mean(rsz_wbt_phase_sh,2);  sem_rsz_wbt_phase_sh = std(rsz_wbt_phase_sh,[],2)/sqrt(N_sweeps_sh);
    avg_rsz_clk_sh = mean(rsz_clk_sh,2);              sem_rsz_clk_sh = std(rsz_clk_sh,[],2)/sqrt(N_sweeps_sh);
    ...
    xticks([-170 0 170]);    ylabel('Accelerometer');    plot(0*[1 1],ylim,'k--'); xlim('tight');           xlim([-170; 170]);
    sgtitle(['Average of ',num2str(N_sweeps_rl),' cycles']);
...
for hide=1
    ...
    %=== Init vectors
    SWPs_posterior = [];  SWPs_wbtphase = [];  SWPs_amp = [];   SWPs_wbtphase_ctrl = [];
    ...
    for zz=1:N_flights
        ...
        for ss=1:numel(candidate_swp)
            if candidate_swp(ss)-template_samples>0 && candidate_swp(ss)+template_samples< size(sft_posterior,2) && round(N_spatial_bins/2)-1*sweep_binspan>0 && round(N_spatial_bins/2)+3*sweep_binspan<N_spatial_bins
                ...
                SWPs_wbtphase_ctrl = [SWPs_wbtphase_ctrl; wbt_phase(randi(numel(wbt_phase)))];
    ...
    phase2wbt_ctr = smoothdata(histcounts(SWPs_wbtphase_ctrl(cond),phase_bins_tmp,'Normalization','probability'),'movmean',bin_smoothing);
