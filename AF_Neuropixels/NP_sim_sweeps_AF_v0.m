...
sigma = 0.4;            % Duration of the firing field, 0.4

pk_fir = 160;           % Proportional to peak firing rate, 150
beta = 0.7;             % Contribution of the wigbeat, 0.8
alpha = 0.9;            % Asymmetry factor 0.3
dt_adapt = 0.09;        % Adaptation time 0.08  
min_spk_adapt = 2;      % Min spikes for adaptation, 2
...
t_dec_bin_dur = 0.020;                                                      % Time bin duration for decoding (def 25ms)
t_dec_bin_ovl = t_dec_bin_dur-0.002;                                        % Time bin overlap for decoding (t_bin_dur-0.005)
...
%=== Plot firing fields
figure('units','normalized','outerposition',[.5 .7 .5 .3]);
for nc= 1:N
...
for nc = 1:min(N,100)
    ...
    yticks(pi*[-1 0 1 2 3]);  yticklabels({'-180', '0', '180', '360', '540'});  axis square;    xticks([]);
end
