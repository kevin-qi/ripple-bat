function [out] = loadTrodesLFP_AF_v1
%% Modified from loadTrodesLFP, originally from KQ. Feb 2024
% Remember that LFP from the Neuropixels headstage is already filtered between 0.5 and 500 (or 600) Hz. Therefore, it can be certainly downsampled to a few kHz or even less. 
...
disp(['Extracting LFP from:', mergedLFP_dirname]);

%=== Load Neural Workspace Trodes file
Trodes_Config_File = dir(fullfile(fileparts(path_to_recording_dir),'*neural_workspace.trodesconf'));
TrodesFileConfig = readTrodesFileConfig([Trodes_Config_File.folder,'\',Trodes_Config_File.name]);
...
%% Compare the channel maps generated here VS those from the Neural Workspace Trodes file

%=== Extract the channel positions for each probe from the Neural Workspace Trodes file
num_channels_tot = size(TrodesFileConfig.nTrodes,2);
Probes_table_Trodes = zeros(num_channels_tot,4);
for i=1:num_channels_tot
    Probes_table_Trodes(i,1) = str2double(TrodesFileConfig.nTrodes(i).id(1));
    Probes_table_Trodes(i,2) = str2double(TrodesFileConfig.nTrodes(i).id(2:end));
    Probes_table_Trodes(i,3) = str2double(TrodesFileConfig.nTrodes(i).channelInfo.coord_ml);
    Probes_table_Trodes(i,4) = str2double(TrodesFileConfig.nTrodes(i).channelInfo.coord_dv);
end

%=== Extract the channel positions for each probe from the local variable out
Probes_table_new = [];
for i=1:nProbes
   Probes_table_new = [Probes_table_new; i*ones(numel(out.channelMap{i}),1), out.channelMap{i}',out.channelPositions{i}];
end  

Probes_table_new = sortrows(Probes_table_new, [1 2]);
Probes_table_Trodes = sortrows(Probes_table_Trodes, [1 2]);

%=== Compare the two 
if all(round(Probes_table_Trodes) == round(Probes_table_new),'all')
    disp('Channel Maps and Locations match with Trodes neural config');
else
    disp('--- WARNING!! --- Channel Maps and Locations DO NOT match with Trodes neural config');
end
...
disp(['Saving LFP at ', num2str(1/mean(diff(t_ds))), ' Hz (downsampled x ', num2str(ds_factor), ')']);
