function [weighted_corr, max_jump_dist, avg_jump_distance,num_peaks, replay_score, slope, fitted_y, posterior_spread, com_jump,segment_len_frac, horizontal_percent,spatial_coverage] = evaluate_candidate_event_v7(event)
    try
    ...
    %% calculating spatial coverage of replay (% of positions covered by max sequence)
    nS = size(event,2);
    nT = size(event, 1);
    positions_occupied = zeros (1,nS);
    position_occupied (decoded_position) = 1;
    position_occupied = conv2(position_occupied, [1 1 1], "same");
    position_occupied = position_occupied > 0;
    spatial_coverage = sum(position_occupied)/nS;
    %% calculating horizontal segment of replay 
