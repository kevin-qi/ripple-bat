function [new_st, new_en] = horiz_segment_v3(event,nan_duration)
    ...
    nS = size(event,2);
    nT = size(event,1);
%     nan_duration = floor (nan_percent*nT);
%     nan_duration = 10;
    uniform_prob = 1/nS;
...
%     horizontal_percent = horiz_sum/size(event,1);
    ...
    long_event_idx = find(event_dur>nan_duration);
    for i = 1:numel(long_event_idx)
        current_event = long_event_idx(i);
        if event_st(current_event) <nT/2
            horiz_segment(1:event_st(current_event)) = 1;
        end

        if event_en(current_event) >nT/2
            horiz_segment(event_en(current_event):nT) = 1;
        end
    end
