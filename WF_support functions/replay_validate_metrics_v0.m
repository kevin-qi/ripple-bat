...
shuffle_times = 5; % this determines how many shuffles 
...
for i = 1:shuffle_times 
    ...
    [weighted_corr_test(i),max_jump_distance_test(i), avg_jump_distance_test(i),num_peaks_test(i)] = evaluate_candidate_event(test_prob_gau_shuffled);
end 
figure; 
subplot(2,2,1); imagesc(test_prob_gau_shuffled'); xlabel("time bins"); ylabel("position");
...
for i = 1:total_shuffles
    ...
    [weighted_corr_test_high_noise(i),max_jump_distance_test_high_noise(i), avg_jump_distance_test_high_noise(i),num_peaks_test_high_noise(i)] = evaluate_candidate_event(test_prob_gau_shuffled);
end
...
for i = 1:total_shuffles
    ...
    [weighted_corr_test_low_noise(i),max_jump_distance_test_low_noise(i), avg_jump_distance_test_low_noise(i),num_peaks_test_low_noise(i)] = evaluate_candidate_event(test_prob_gau_shuffled);
end
...
fig = figure;
subplot(1,3,1);hist([max_jump_distance_test_high_noise' max_jump_distance_test_low_noise']); xlabel("Maximum jump distance"); ylabel("count");
legend('high noise','low noise','Location','northwest')
subplot(1,3,2);hist([weighted_corr_test_high_noise' weighted_corr_test_low_noise']); xlabel("Weighted correlation"); ylabel("count");
subplot(1,3,3); hist([avg_jump_distance_test_high_noise' avg_jump_distance_test_low_noise']);  xlabel("Average jump distance");  ylabel("count");
